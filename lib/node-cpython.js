...
* @class Ncpy
*/
function Ncpy (options) {
  EventEmitter.call(this)
  // default options
  this.opts = {
    version: '2.7' // 2.7 or '3.x'
  }
  // copy options
  if (options) {
...
Ncpy.prototype.simpleString = function (str, cb) {
  ...
  if (typeof str !== 'string' || str instanceof String) {
    err = new Error('Method `simpleString` expects at least one argument, which must be a string and optionally a callback')
    err.name = 'InputError'
    this.emit('error', err)
    return cb(err)
  }
...
Ncpy.prototype.ffi = function (file, functionname, argList, cb) {
  ...
  if (args.length === 0) {
    err = new Error('Method `.ffi()` expects at least 3 arguments.')
    err.name = 'InputError'
    this.emit('error', err)
    return
  }
...
/**
 * You can pass streams as objects into pipe, so your stream (the object)
 * receives the chunk as first argument.
 * @param  {Object} stream instantated stream; probably a transform a writable stream
 * @return {Object}        returns itself to be chainable
 */
Ncpy.prototype.ffi.pipe = function (stream) {
  FFI._pipe(stream)
  return this
}
